(function(){
    ...
    */
    ...
    function hasClass(markup, cssClass) {
        var cssRegex = new RegExp(cssClass,'g');
        var regexHtmlCss = new RegExp('<(\\w+.?\\w+)="(\\w+.?\\w+)">','g');

        return(markup.match(regexHtmlCss).some(function(value1){
            return value1.match(cssRegex);
        }));  
    }
